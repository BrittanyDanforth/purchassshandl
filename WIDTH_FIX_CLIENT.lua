--[[
    ...
    ║                    FIXES UI WIDTHS - NO MORE OVERLAPPING!                            ║
    ╚══════════════════════════════════════════════════════════════════════════════════════╝
]]

-- The issue: UIs are too WIDE, not positioned wrong!
-- MainPanel already accounts for navigation (80px), but UIs use full width
...
print("[SanrioTycoonClient] Starting WIDTH FIX client v7.2...")
...
-- CONSTANTS
-- ========================================
-- Standard UI size that works well (like Shop)
local STANDARD_UI_WIDTH = 800  -- Fixed width instead of full width
local STANDARD_UI_HEIGHT = 600  -- Fixed height
local STANDARD_UI_SIZE = UDim2.new(0, STANDARD_UI_WIDTH, 0, STANDARD_UI_HEIGHT)
local STANDARD_UI_POSITION = UDim2.new(0.5, -STANDARD_UI_WIDTH/2, 0.5, -STANDARD_UI_HEIGHT/2)  -- Centered
...
-- ========================================
-- FIX UI FACTORY TO USE PROPER SIZES
-- ========================================
...
-- Override CreateFrame to use standard sizes
...
if originalCreateFrame then
    uiFactory.CreateFrame = function(self, parent, config)
        -- If it's a main UI frame, use standard size
        if config.name and (config.name:match("Frame$") or config.name:match("UI$")) then
            -- Don't use full width!
            if config.size and config.size.X.Scale >= 1 then
                config.size = STANDARD_UI_SIZE
                config.position = STANDARD_UI_POSITION
                print("[UIFactory] Fixed " .. config.name .. " to use standard size")
            end
        end
...
-- ========================================
-- UI MODULES WITH WIDTH FIXES
-- ========================================
print("[SanrioTycoonClient] Loading UI modules with width fixes...")
    ...
    if originalCreateUI then
        instance.CreateUI = function(self)
            ...
            if self.Frame then
                self.Frame.Size = STANDARD_UI_SIZE
                self.Frame.Position = STANDARD_UI_POSITION
                print("[QuestUI] Fixed size to standard")
            end
        end
    end
...
-- WIDTH FIX FUNCTION
-- ========================================
local function fixAllUIWidths()
    local screenGui = PlayerGui:FindFirstChild("SanrioTycoonUI")
    if not screenGui then return end
    
    print("[SanrioTycoonClient] Applying width fixes...")
    
    -- Find and fix all UI frames
    local framesToFix = {
        "InventoryFrame",
        "QuestFrame",
        "SettingsFrame",
    ...
    for _, frameName in ipairs(framesToFix) do
        ...
        if frame and frame:IsA("Frame") then
            -- Use standard size instead of full width
            frame.Size = STANDARD_UI_SIZE
            frame.Position = STANDARD_UI_POSITION
            frame.AnchorPoint = Vector2.new(0.5, 0.5)  -- Center anchor
            print("[SanrioTycoonClient] Fixed " .. frameName .. " width")
        end
    end
end
...
task.spawn(function()
    ...
    -- Apply width fixes again
    fixAllUIWidths()
    
    eventBus:Fire("ClientReady")
    
    print("[SanrioTycoonClient] ========================================")
    print("[SanrioTycoonClient] ✅ WIDTH FIX CLIENT v7.2 READY!")
    print("[SanrioTycoonClient] ========================================")
end)
...
_G.SanrioTycoonClient = {
    ...
    Debug = {
        FixWidths = function()
            fixAllUIWidths()
            print("Applied width fixes to all UIs")
        end,
        ...
        PrintSizes = function()
            ...
            if screenGui then
                print("=== UI Sizes ===")
                print("Navigation Width: 80px")
                print("Standard UI Size: " .. STANDARD_UI_WIDTH .. "x" .. STANDARD_UI_HEIGHT)
                for _, child in ipairs(screenGui:GetDescendants()) do
                    if child:IsA("Frame") and (child.Name:match("Frame$") or child.Name:match("UI$")) then
                        print(child.Name .. " - Size: " .. tostring(child.Size))
                    end
                end
            end
        end,
...
print("[SanrioTycoonClient] 🎉 WIDTH FIX v7.2 LOADED!")
print("[SanrioTycoonClient] 📦 " .. loadedCount .. "/" .. #uiModuleNames .. " UI modules")
print("[SanrioTycoonClient] 🔧 Fixes:")
print("[SanrioTycoonClient]   ✅ UIs use 800x600 standard size")
print("[SanrioTycoonClient]   ✅ UIs centered in MainPanel")
print("[SanrioTycoonClient]   ✅ No more full-width overlapping")
print("[SanrioTycoonClient]   ✅ Case opening animations fixed")
