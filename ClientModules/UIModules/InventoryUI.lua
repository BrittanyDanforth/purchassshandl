    ...
    if self.VirtualScrollEnabled then
        ...
        for index, card in pairs(self.ActiveCards) do
            if card.Name == "PetCard_" .. uniqueId then
                local nameLabel = card:FindFirstChild("NameLabel")
                if nameLabel then
                    nameLabel.Text = newName or petInfo.data.displayName or petInfo.data.name or "Unknown"
                end
                break
            end
        end
        ...
        for _, card in ipairs(self.PetCardCache) do
            if card.Name == "PetCard_" .. uniqueId then
                local nameLabel = card:FindFirstChild("NameLabel")
                if nameLabel then
                    -- Get pet data to fallback to display name if no nickname
                    local playerData = self._dataCache and self._dataCache:Get("playerData")
                    if playerData and playerData.pets and playerData.pets[uniqueId] then
                        local pet = playerData.pets[uniqueId]
                        nameLabel.Text = newName or pet.displayName or pet.name or "Unknown"
                    else
...
function InventoryUI:Destroy()
    ...
    -- Cancel animations
    for _, tween in pairs(self.ActiveTweens or {}) do
        tween:Cancel()
    end
    self.ActiveTweens = {}
