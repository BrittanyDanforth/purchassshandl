--[[
    ...
    ║                          NO DUPLICATE UIs - ALL FIXES INCLUDED                        ║
    ╚══════════════════════════════════════════════════════════════════════════════════════╝
]]
...
if _G.SanrioTycoonClientLoaded then
    warn("[SanrioTycoonClient] Already loaded! Force reloading...")
    -- Clean up old instance
    if _G.SanrioTycoonClient then
        if _G.SanrioTycoonClient.Cleanup then
            _G.SanrioTycoonClient:Cleanup()
        end
...
print("[SanrioTycoonClient] Starting ULTIMATE FIXED client...")
...
local function safeRequire(module)
    ...
    if loadingModules[module] then
        warn("[SanrioTycoonClient] Circular dependency detected:", module.Name)
        return nil
    end
...
local function initializeUI()
    if hasInitialized then
        warn("[SanrioTycoonClient] UI already initialized!")
        return
    end
    hasInitialized = true
    ...
    -- Wait for ScreenGui to be created
    task.wait(0.1)
    
    -- Find the ONE ScreenGui
    screenGui = PlayerGui:FindFirstChild("SanrioTycoonUI")
    
    if screenGui and windowManager then
...
task.spawn(function()
    ...
    print("[SanrioTycoonClient] ✅ ULTIMATE FIXED CLIENT READY!")
    print("[SanrioTycoonClient] ========================================")
    print("[SanrioTycoonClient] Loaded " .. table.getn(uiModules) .. " UI modules")
    print("[SanrioTycoonClient] ScreenGui: " .. (screenGui and "Created" or "Missing"))
end)
    ...
    Debug = {
        ...
        CheckDuplicates = function()
            ...
            for _, gui in ipairs(PlayerGui:GetChildren()) do
                if gui.Name == "SanrioTycoonUI" then
                    count = count + 1
                    print("Found SanrioTycoonUI:", gui)
                end
            end
...
-- Update ScreenGui reference when available
task.spawn(function()
    task.wait(1)
