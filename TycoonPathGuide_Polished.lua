--[[
    Polished Tycoon Path Guide System
    - Beautiful glowing path on ground
    - No floating or buggy behavior
...
-- Update path for a player
local function updatePlayerPath(player)
    local character = player.Character
    if not character then
        if playerPaths[player] then
            playerPaths[player]:Destroy()
    ...
    if ownsATycoon or not nearestGate then
        if playerPaths[player] then
            -- Fade out smoothly
            local model = playerPaths[player]
            for _, segment in pairs(model:GetChildren()) do
            ...
            Debris:AddItem(model, FADE_TIME)
            playerPaths[player] = nil
        end
        return
    end
    ...
    if nearestDistance < MIN_DISTANCE or nearestDistance > MAX_DISTANCE then
        if playerPaths[player] then
            for _, segment in pairs(playerPaths[player]:GetChildren()) do
                if segment:IsA("Part") then
    ...
    -- Create or get path model
    local pathModel = playerPaths[player]
    if not pathModel or not pathModel.Parent then
        pathModel = Instance.new("Model")
        pathModel.Name = player.Name .. "_TycoonPath"
        pathModel.Parent = workspace
        playerPaths[player] = pathModel
    end
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    -- Update all player paths
    for player, pathModel in pairs(playerPaths) do
    ...
    -- Update paths every frame for smooth movement
...
print("✅ Polished Tycoon Path Guide loaded!")
print("✨ Features: Smooth ground tracking, beautiful animations, no floating!")
print("🎯 Path appears when unclaimed tycoons exist")